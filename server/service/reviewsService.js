...
const getFilterReviews = async (options, page) => {
  ...
  Object?.entries(options)?.map(([key, value]) => {
    if (key !== '' && key !== 'total' && value !== '' && value !== 'total' && key !== 'searchTerm') {
      query[key] = value
    }
  })

  let reviews = await Review.find(query).skip(skipIndex).limit(itemsPerPage).populate('productId').populate('userId')
  ...
  if (searchTerm?.length) {
    reviews = reviews.filter(review => {
      ...
      const email = review.userId?.email?.toLowerCase() || ''
      const model = review.productId?.model?.toLowerCase() || ''
